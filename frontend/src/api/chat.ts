...
/** Delete one chat session (`/api/v1/chat/sessions/delete`). The backend
 *  resolves the session by exact id and 404s when it doesn't exist (surfaced
 *  as an `ApiError` by `apiFetch`). */
export function deleteSession(kb: string, sessionId: string): Promise<{ deleted: boolean }> {
  return apiFetch<{ deleted: boolean }>("/api/v1/chat/sessions/delete", {
    body: { kb, session_id: sessionId },
  })
}
