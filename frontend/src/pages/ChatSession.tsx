...
import { ArrowLeft, FileText, FolderInput, Loader2, Sparkles, BookText, CheckCircle2, CircleStop, Trash2 } from "lucide-react"
...
export default function ChatSession() {
  ...
  // Session deletion: the header trash button toggles a confirm state ("Delete?"
  // in place of the icon). Confirming calls the backend and navigates home on
  // success; cancelling (or navigating away) resets it.
  const [confirmDelete, setConfirmDelete] = useState(false)
  const [deleting, setDeleting] = useState(false)
  const deleteCurrentSession = async () => {
    const sid = sessionIdRef.current
    const activeKb = kbRef.current
    if (!sid || !activeKb || deleting) return
    setDeleting(true)
    try {
      await deleteSession(activeKb, sid)
      toast.success(t("chat:sessionDeleted"))
      navigate("/")
    } catch (e) {
      setConfirmDelete(false)
      toast.error(t("chat:errors.deleteSession", { error: errMsg(e) }))
    } finally {
      setDeleting(false)
    }
  }
  ...
  return (
    <div className="h-full flex">
      ...
      <div className="shrink-0 h-12 flex items-center gap-3 px-5 border-b border-[hsl(var(--glass-border))] glass-2 backdrop-blur">
        ...
        {/* 删除会话：仅对已保存的会话显示（/chat/new 没有可删的 id）；确认态显示“删除？”文本按钮，点击调用 /sessions/delete */}
        {id !== "new" && id && (
          ...
          {confirmDelete ? (
            <div className="flex items-center gap-1.5">
              <button
                ...
                disabled={deleting}
                className="inline-flex items-center h-7 px-2.5 rounded-lg bg-red-600 text-white text-[12px] font-medium hover:bg-red-500 active:scale-[0.97] transition duration-fast disabled:opacity-60"
              >
                {deleting ? <Loader2 className="w-3 h-3 animate-spin" /> : t("chat:deleteConfirm")}
              </button>
              <button
                type="button"
                onClick={() => setConfirmDelete(false)}
                disabled={deleting}
                className="inline-flex items-center h-7 px-2.5 rounded-lg border border-[hsl(var(--glass-border))] glass-2 text-[12px] font-medium text-muted-foreground hover:text-foreground hover:border-accent-brand/40 transition-colors"
              >
                {t("chat:deleteCancel")}
              </button>
            </div>
            ...
            <button
              ...
              className="w-7 h-7 rounded-lg grid place-items-center text-muted-foreground hover:bg-accent hover:text-red-500 transition-colors"
            >
