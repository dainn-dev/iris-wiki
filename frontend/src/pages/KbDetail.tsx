...
/** Format a whole-seconds duration for the upload timer, e.g. `42s` or `1m 05s`. */
...
export default function KbDetail() {
  ...
  // Elapsed-time measurement for the upload: `uploadStartedAt` is recorded the
  // moment files are selected, and `uploadElapsedSec` ticks every second while
  // `uploading` so the UI shows live progress; the final value stays after the
  // upload completes to show total time-from-upload-to-finish.
  const [uploadStartedAt, setUploadStartedAt] = useState<number | null>(null)
  const [uploadElapsedSec, setUploadElapsedSec] = useState(0)
  useEffect(() => {
    if (!uploading || uploadStartedAt == null) return
    const id = setInterval(
      () => setUploadElapsedSec(Math.round((Date.now() - uploadStartedAt) / 1000)),
      1000,
    )
  ...
  const doUpload = useCallback(
    async (files: File[]) => {
      ...
      try {
        ...
        setUploading(false)
        // Pin the exact duration now that the interval has stopped (it could
        // otherwise lag by up to one tick).
        if (uploadStartedAt != null) {
          setUploadElapsedSec(Math.round((Date.now() - uploadStartedAt) / 1000))
        }
      }
    },
    [id, uploading, refreshInventory, t, uploadStartedAt],
  )
            ...
            <div className="flex items-center justify-between">
              ...
              {uploadStartedAt != null && (
                <span className="text-[11.5px] text-muted-foreground tabular-nums shrink-0">
                  {uploading
                    ? t('kb:upload.elapsedRunning', { time: formatDuration(uploadElapsedSec) })
                    : t('kb:upload.elapsedDone', { time: formatDuration(uploadElapsedSec) })}
                </span>
              )}
            </div>
